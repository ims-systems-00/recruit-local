import { CertificationResponseDto } from "@rl/types";
...
/**
 * Serializes a (possibly partial) sanitized certification into its public HTTP
 ...
 */
export const toCertificationResponse = (doc: unknown): CertificationResponseDto => {
  ...
  if (has(d, "issueDate")) d.issueDate = d.issueDate == null ? null : toIso(d.issueDate);
  if (has(d, "createdAt")) d.createdAt = toIso(d.createdAt);
  if (has(d, "updatedAt")) d.updatedAt = toIso(d.updatedAt);

  return d as CertificationResponseDto;
};

export const toCertificationResponseList = (docs: unknown[]): CertificationResponseDto[] =>
  docs.map(toCertificationResponse);
