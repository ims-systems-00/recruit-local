...
import { buildRankingContext, computeRanking } from "./ranking.service";
...
export const list = ({ query = {}, options, session }: IListApplicationParams) => {
  ...
  // Rank best-fit first by default; caller can still override with ?sort=
  const opts = { ...options };
  if (!opts.sort || Object.keys(opts.sort).length === 0) opts.sort = { matchScore: -1, createdAt: -1 };

  return Application.aggregatePaginate(aggregate, opts);
};
...
export const create = async ({ payload, session }: IApplicationCreateParams) => {
  ...
  // Compute the match/ranking score from the applicant's values, answers and experience.
  const { matchScore, breakdown } = computeRanking(
    await buildRankingContext({
      jobId: payload.jobId,
      jobProfileId: payload.jobProfileId,
