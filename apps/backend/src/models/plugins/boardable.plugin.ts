  ...
  schema.static(
    ...
    async function (
      ...
      return withTransaction(async (session: ClientSession) => {
        ...
        const parentDoc = await ParentModel.findById(parentId).select("boardSortBy").session(session);

        if (!parentDoc) throw new Error(`${options.parentModelName} not found`);

        // GUARD: If board is not sorted by rank, disable manual moving
        if (parentDoc.boardSortBy && parentDoc.boardSortBy !== "rank") {
          throw new Error(`Cannot manually move items when sorted by ${parentDoc.boardSortBy}`);
        }

        const targetStatusObjectId = new Types.ObjectId(targetStatusId);

        const statusExists = await Status.exists({ _id: targetStatusObjectId }).session(session);
        if (!statusExists) throw new Error("Target status not found");
