...
export async function getCertifications(
  ...
  try {
    ...
    const backendResponse = await certificationListResponseSchema.validate(
      res.data,
      {
        stripUnknown: true,
      },
...
export async function getCertificationById(
  ...
  try {
    ...
    const res = await axiosServer.get<CertificationItemBackendResponse>(
      `${API_ENDPOINT}/${id}`,
    );

    const backendResponse = await certificationItemResponseSchema.validate(
      res.data,
      {
        stripUnknown: true,
      },
...
export async function hardDeleteCertification(
  ...
  try {
    ...
    return handleServerError(
      error,
      'Failed to permanently remove certification',
    );
  }
}
