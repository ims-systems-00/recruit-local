...
export function useCreateCv() {
    ...
    const mutation = useMutation({
        ...
        onError: (error) => toast.error(error.message || 'Failed to create CV'),
    });

    return { createCv: mutation.mutateAsync, isLoading: mutation.isPending };
}
...
export function useUpdateCv() {
    ...
    const mutation = useMutation({
        ...
        onError: (error) => toast.error(error.message || 'Failed to update CV'),
    });

    return { updateCv: mutation.mutateAsync, isLoading: mutation.isPending };
}
