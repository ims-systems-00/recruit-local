...
import { kycKeys, useCreateKyc, useKycList } from '@/services/kyc/kyc.client';
...
import VerificationSkelaton from './verification-skelaton';
...
export default function Verification({ profileUid }: { profileUid: string }) {
  const { kyc, isLoading: isLoadingKyc } = useKycList({
    limit: 1,
  });
  ...
  if (isLoadingKyc) {
    return <VerificationSkelaton />;
  }

  const kycData = kyc?.[0];

  if (kycData) {
    return <VerificationPreview kycData={kycData} />;
  }
