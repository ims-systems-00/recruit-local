...
export class InteractionAuthZEntity {
  constructor(public readonly tenantId: string | null = null) {}
}

type ClaimAbility = PureAbility<
  AbilityTuple,
  MongoQuery<typeof InteractionAuthZEntity>
>;
const ClaimAbility = PureAbility as AbilityClass<ClaimAbility>;

export class InteractionAbilityBuilder implements IAbilityBuilder {
  ...
  getAbility(): AnyAbility {
    ...
    switch (this.session.user.type) {
      case USER_TYPE_ENUMS.PLATFORM_ADMIN:
        builder.cannot(AbilityAction.Manage, InteractionAuthZEntity);
        break;
